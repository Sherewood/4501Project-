Shader "Custom/TargetedEnemyShader"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            //brackets
            if((abs(pos.x) < 0.02 && (pos.y >= 0.4 || pos.y <= -0.4)) ||
               (abs(pos.y) < 0.02 && (pos.x >= 0.4 || pos.x <= -0.4))){
                o.Albedo = float3(0.7,0.0,0.0);
                o.Alpha = 1.0;
            }
            //ring
            else if(diff < 0.035){
                o.Albedo = float3(1.0,0,0);
                o.Alpha = smoothstep(0.035,0.0,diff);
            }
            //soulless void
            else {
                o.Alpha = 0.0;
